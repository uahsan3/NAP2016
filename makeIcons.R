...
schoolIcon <- makeIcon(
  iconUrl = ifelse(schools$Grades == "Elementary ", paste(icon_folder, "school-p.png", sep = "/"), 
                   ifelse(schools$Grades == "Middle", paste(icon_folder, "school-m.png", sep = "/"), 
                          ifelse(schools$Grades == "High", paste(icon_folder, "school-h.png", sep = "/"), paste(icon_folder, "school-2.png", sep = "/")))),
  iconWidth = 28, iconHeight = 30,
  iconAnchorX = 5, iconAnchorY = 20
)

faithIcons <- icons(
  iconUrl = ifelse(Faith$place_type == "church", paste(icon_folder, "cross-2.png", sep = "/"),
                   ifelse(Faith$place_type == "mosque", paste(icon_folder, "mosque.png", sep = "/"),
                          ifelse(Faith$place_type == "synagogue", paste(icon_folder, "synagogue-2.png", sep = "/"), 
                                 ifelse(Faith$place_type == "hindu_temple", paste(icon_folder, "templehindu.png", sep = "/"),
